...
PPU::PPU() {
    std::string palletteFileName = "res/QuickNes.pal";
    std::ifstream ifs;
    ifs.open(palletteFileName, std::ifstream::binary);
    if (ifs.is_open()) {
        auto color = SDL_Color();
        for (int i = 0; i < 0x40; i++) {
            ifs.read((char*)&color, sizeof(uint8_t)*3);
            palScreen[i] = color;
        }
    } else {
        std::cout << "Failed to open " << palletteFileName << std::endl;
    }
}
